...
RSpec.describe EvaluatorInvitation, type: :model do
  ...
  it "can be created with valid attributes" do
    invitation = build(:evaluator_invitation, challenge: challenge, phase: phase)
    expect(invitation).to be_valid
    expect { invitation.save! }.to change(EvaluatorInvitation, :count).by(1)
  end

  it "can be destroyed" do
    invitation = create(:evaluator_invitation, challenge: challenge, phase: phase)
    expect { invitation.destroy }.to change(EvaluatorInvitation, :count).by(-1)
  end

  it "validates uniqueness of email within challenge and phase" do
    create(:evaluator_invitation, challenge: challenge, phase: phase, email: "test@example.com")
    duplicate_invitation = build(:evaluator_invitation, challenge: challenge, phase: phase, email: "test@example.com")
  ...
  it "allows the same email for different challenges or phases" do
    ...
    invitation_different_challenge = build(:evaluator_invitation, challenge: different_challenge, phase: phase, email: "test@example.com")
    expect(invitation_different_challenge).to be_valid

    invitation_different_phase = build(:evaluator_invitation, challenge: challenge, phase: different_phase, email: "test@example.com")
