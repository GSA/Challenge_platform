# Challenge.Gov Platform cloud.gov Guide
...
1. Within each space, this platform is designed to run alongside the portal application. Create the platform app as an empty shell of an application, no template. See below for details.

## Application configuration

1. Name the application based on the environment, for example: challenge-dev
1. Bind the default route using the cloud.gov domain
1. Bind to the PostgreSQL database the portal is using.
1. Bind a S3 bucket the portal is using
...
1. There are 3 branches that cover all environments for automatic deployment. At this time, only dev is set up, see the portal code circle config for approach to deploy the platform to higher environments.
    ...
    - `production` will deploy to the production cloud.gov space
...
The app will run database migrations when booting. Background tasks are processed by the portal.

## Proxy

This app expects the configuration of an egress proxy. The configuration is managed locally with a sidecar as well as through the deployment of an egress proxy application configured in this codebase.
...
```
cf ssh challenge-dev -L 5433:RDS_HOST:5432
```
